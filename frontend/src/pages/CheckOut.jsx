...
const CheckOut = () => {
  ...
  const getAddressByLatLng = async (lat, lng) => {
    try {
      ...
      console.log(error, "error aagya fetching me")
    }
  }
  ...
  const handlePlaceOrder = async()=>{
    try{
      ...
      // console.log("placeorder me error aagya",error.message)
  ...
  return (
    <div className='min-h-screen bg-[#fff9f6] flex items-center justify-center p-6'>
      <div className='absolute top-[20px] left-[20px] z-[10]' onClick={() => navigator("/")}>
        <IoMdArrowBack size={35} className='text-[#ff4d2d]'></IoMdArrowBack>
      </div>
      <div className='w-full max-w-[900px] bg-white rounded-2xl shadow-xl p-6 space-y-6'>
        <h1 className='text-2xl font-bold text-gray-800'>CheckOut</h1>
        {/* map section  */}
        <section>
          <h2 className='text-lg font-semibold mb-2 flex items-center gap-2 text-gray-800'><IoLocation size={16} className='text-[#ff4d2d]'></IoLocation>Delivery Location</h2>
          <div className='flex gap-2 mb-3'>
            ...
            <button className='bg-[#ff4d2d] hover:bg-[#e64526] text-white px-3 py-2 rounded-lg flex items-center justify-center' onClick={getLatLngByAddress}><IoMdSearch></IoMdSearch></button>
            <button className='bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg flex items-center justify-center' onClick={getCurrentLocation}><BiCurrentLocation></BiCurrentLocation></button>
          </div>
          ...
          <div className='rounded-xl border overflow-hidden'>
            <div className='h-64 w-full flex items-center justify-center'>
              <MapContainer
                ...
                />
                <RecenterMap location={location}></RecenterMap>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            <div className={`flex items-center gap-3 rounded-xl border p-4 text-left transition ${paymentMethod === "cod"?"border-[#ff4d2d] bg-orange-50 shadow" : "border-gray-200 hover:border-gray-300"}`} onClick={()=>setPaymentMethod("cod")}>
              <span className='inline-flex h-10 w-10 items-center justify-center rounded-full bg-green-100'>
                <MdDeliveryDining className='text-green-600 text-xl'></MdDeliveryDining>
              </span>
              <div>
            ...
            <div className={`flex items-center gap-3 rounded-xl border p-4 text-left transition ${paymentMethod === "online"?"border-[#ff4d2d] bg-orange-50 shadow" : "border-gray-200 hover:border-gray-300"}`} onClick={()=>setPaymentMethod("online")}>
              <span className='inline-flex h-10 w-10 items-center justify-center rounded-full bg-purple-100'><FaMobileAlt className='text-purple-700 text-lg'></FaMobileAlt></span>
              <span className='inline-flex h-10 w-10 items-center justify-center rounded-full bg-purple-100'><FaRegCreditCard className='text-blue-700 text-lg'></FaRegCreditCard></span>
              <div>
